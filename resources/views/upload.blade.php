        ...
        <div class="card mt-4">
            ...
            <div class="card-header">
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                    ...
                    {{-- GRUP TOMBOL (Reset, Download & Import) --}}
                    <div class="d-flex justify-content-end align-items-center gap-2">

                        {{-- 1. TOMBOL RESET DATA --}}
                        {{-- Beri ID unik pada form, misal: id="formResetDb" --}}
                        <form id="formResetDb" action="{{ route('upload.truncate') }}" method="POST">
                            @csrf
                            {{-- Ubah type menjadi "button" agar tidak langsung submit --}}
                            {{-- Tambahkan onclick untuk memanggil fungsi alert --}}
                            <button type="button" onclick="confirmReset()" class="btn btn-outline-danger btn-sm" style="width: 170px;">
                                <i class="bx bx-trash-alt me-1"></i> Reset DB
                            </button>
                        </form>

                        {{-- 2. Tombol Download --}}
                        {{-- Tambahkan style min-width --}}
                        <a href="{{ route('upload.export.belumjmo', ['search' => request('search')]) }}" class="btn btn-success btn-sm" style="min-width: 170px;">
                            <i class="bx bx-download me-1"></i> Excel Belum JMO
                        </a>

                        {{-- 3. Tombol Import --}}
                        {{-- Tombol Import dengan Style SweetAlert --}}
                        <button type="button" onclick="openImportModal()" class="btn btn-primary btn-sm" style="width: 170px;">
                            <i class="bx bx-upload me-1"></i> Import Baru
                        </button>
                ...
                <table class="table table-hover">
                    <thead class="table-light">
                        <tr>
                            ...
                            {{-- PERUBAHAN DI SINI: GANTI SEGMEN JADI PEMBINA --}}
                            <th>Pembina</th>
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            ...
                            {{-- PERUBAHAN DI SINI: PANGGIL KOLOM PEMBINA --}}
                            <td>
                                {{ $item->nama_pembina ?? '-' }}
                            </td>
                            ...
                            <td class="text-center">
                                {{-- 1. Berikan ID unik menggunakan $item->id --}}
                                <form id="delete-form-{{ $item->id }}" action="{{ route('upload.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')

                                    {{-- 2. Ubah type="button" dan tambahkan onclick dengan ID spesifik --}}
                                    <button type="button" onclick="confirmDelete({{ $item->id }})" class="btn btn-icon btn-sm btn-outline-danger" title="Hapus Data">
